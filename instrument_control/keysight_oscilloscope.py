...
class KeysightDSOX6004A:
    ...
    def measure_single(self, channel: int, measurement_type: str) -> Optional[float]:
        ...
        try:
            ...
            try:
                ...
                if "Timeout" in str(e) or "TMO" in str(e):
                    self._logger.warning(f"Measurement timeout on Ch{channel} ({measurement_type}) - likely no signal, returning 0.0")
                    return 0.0
                else:
                    raise
    ...
    def configure_function_generator(self, generator: int, waveform: str = "SIN",
        ...
        """
        Configure function generator output with comprehensive waveform support.

        Args:
            generator: Generator number (1 or 2 for WGEN1/WGEN2)
            waveform: Waveform type (see list below)
        ...
        Supported Waveforms:
            Standard: SIN, SINE, SQU, SQUARE, RAMP, PULS, PULSE, DC, NOIS, NOISE
            Arbitrary: ARB, SINC, EXPR, ERISE, EXPF, EFALL, CARD, CARDIAC, GAUS, GAUSPULSE
