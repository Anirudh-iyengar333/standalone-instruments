...
class KeysightDSOX6004A:
    ...
    def autoscale(self) -> bool:
        ...
        The autoscale function automatically adjusts vertical scale, horizontal timebase,
        and trigger level to optimally display all active channel waveforms on screen.
        This is equivalent to pressing the "Auto Scale" button on the physical instrument.
        
        Returns:
            bool: True if autoscale executed successfully, False otherwise
        
        Notes:
            - Autoscale operation typically takes 1-3 seconds to complete
            - All active channels will be optimized simultaneously
            - Previous manual settings will be overwritten
            - Trigger mode will be set to AUTO if not already
        """
        if not self.is_connected:
        ...
        try:
            ...
            # Wait for autoscale operation to complete (typically 1-3 seconds)
            time.sleep(2.0)
            
            # Query operation complete status to ensure autoscale finished
            self._scpi_wrapper.query("*OPC?")
    ...
    # ============================================================================
    # REMAINING METHODS (Function Generator, Screenshot, etc.)
    # ============================================================================
